...
static bool IsPosition( const char *name )
{
	return strcmp( "position", name ) == 0;
}

static bool IsNormal( const char *name )
{
	return strcmp( "normal", name ) == 0;
}

static bool IsTexcoord( const char *name )
{
	return strcmp( "map1", name ) == 0 || strcmp( "lightmap1", name ) == 0;
}
...
template < class T >
static void OutputStream( std::vector<T> &a, const char *stream, int num, int stride, float scale )
{
	char *work = strdup( stream );
...
bool LoadAtgi( std::vector< ModelGeometry * > &geoms, std::vector< std::string > &materials, std::vector< std::string > &surfaceNames, const char *filename )
{
	char buffer[512];
	char *xml;
	if ( !PHYSFS_readFile( fs::MakeCanonicalForm( buffer, filename ), (void**)&xml ) )
	{
		return false;
	}
	...
	if ( result )
	{
		pugi::xml_node scene = doc.child("ATG").child("scene");
		if ( scene )
		{
			for (pugi::xml_node node = scene.child("node"); node; node = node.next_sibling("node"))
			{
				pugi::xml_node mesh = node.child("mesh");
				if ( mesh )
				{
					ModelGeometry *geom = new ModelGeometry;
					
					geoms.push_back( geom );
					materials.push_back( node.attribute( "name" ).value() );
					surfaceNames.push_back( std::string("||") + std::string(node.attribute( "name" ).value()) + std::string("|") +std::string(mesh.attribute( "name" ).value()) );

					pugi::xml_node vertexArray = mesh.child("vertexArray");
					if ( vertexArray )
					{
						...
						texcoords[0].push_back( vec2() );
						texcoords[1].push_back( vec2() );
						...
						int texCoordIdx = 0;
						int arrays = 0;
						const char *arrayNames[16];
						for (pugi::xml_node a = vertexArray.child("array"); a; a = a.next_sibling("array"))
						{
							int c = a.attribute("count").as_int();
							int s = a.attribute("stride").as_int();
							const char *name = a.attribute("name").value();
							const char *data = a.child_value();
							arrayNames[arrays] = name;
							if ( IsPosition( name ) )
							{
								OutputStream( points, data, c/s, s, 100.f );
							} else
							if ( IsNormal( name ) )
							{
								OutputStream( normals, data, c/s, s, 1.f );
								//arrayInclude[arrays] = true;
								//OutputStream( f, data, "vn", c/s, s, 1.f );
								//arrayIdx[arrays] = vnIdx;
								//vnIdx += c/s;
							} else
							if ( IsTexcoord( name ) )
							{
								OutputStream( texcoords[texCoordIdx], data, c/s, s, 1.f );
								texCoordIdx++;
								//arrayInclude[arrays] = true;
								//OutputStream( f, data, "vt", c/s, s, 1.f );
								//arrayIdx[arrays] = vtIdx;
								//vtIdx += c/s;
							}
							arrays++;
						}
						pugi::xml_node atgiindices = vertexArray.child("primitives").child("indices");
						if ( atgiindices )
						{						
							const char *data = atgiindices.child_value();
							...
							while ( tok )
							{
								for (int j=0; j<3; j++)
								{
									...
									{
										int v = atoi( tok ) + 1;
										if ( IsPosition( arrayNames[i] ) )
										{
											output[0] = v;
										} else
										if ( IsNormal( arrayNames[i] ) )
										{
											output[1] = v;
										} else
										if ( IsTexcoord( arrayNames[i] ) )
										{
											output[2+texCoord] = v;
											texCoord++;
										}
									...
									ModelVert v0 = MakeModelVert( points[output[0]], normals[output[1]], texcoords[0][output[2]], texcoords[1][output[3]] );
