...
const var &FindVar( object const *top, int key, vmstate &state )
{
	...
	while ( o )
	{
		std::map<int,var>::const_iterator f = o->m_tbl.find( key );
		if ( f != o->m_tbl.cend() )
		{
			return f->second;
		}
		...
		if ( o->prototype != -1 )
		{
			std::map<int,var>::const_iterator p = state.globals.find(o->prototype);
			if ( p != state.globals.cend() )
			{
...
void RunVM( std::vector<int> const &ops, int loc, vmstate &state )
{
	{
