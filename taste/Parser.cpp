...
void Parser::ProcDecl(ProcDeclAst* &procDecl) {
		VarDeclAst *varDecl; int type; AstBase *stat; DeclVec declVec; EmbedDeclAst *structDecl; 
		Expect(31 /* "function" */);
		...
		while (StartOf(4)) {
			if (la->kind == 16 /* "var" */) {
				...
				StructDefDecl(structDecl);
				procDecl->AddBody( structDecl ); 
			} else {
				Stat(stat);
...
void Parser::StructDefDecl(EmbedDeclAst *&embed) {
		Expect(42 /* "struct" */);
		Expect(_ident);
...
void Parser::Def(DefDecl *&def) {
		...
		while (StartOf(7)) {
			if (la->kind == 16 /* "var" */) {
				...
				StructDefDecl(embedDecl);
				def->AddStructDecl( embedDecl ); 
			} else {
				ProcDecl(procDecl);
...
void Parser::Struct(DefDecl *&def) {
		...
		def = new DefDecl( t->val ); 
		Expect(13 /* "{" */);
		while (la->kind == 16 /* "var" */ || la->kind == 42 /* "struct" */) {
			if (la->kind == 16 /* "var" */) {
				VarDecl(varDecl);
				def->AddVarDecl( varDecl ); 
			} else {
...
bool Parser::StartOf(int s) {
	...
	static bool set[9][47] = {
		...
		{x,x,x,x, x,T,T,T, x,T,T,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x},
		{x,T,x,x, x,x,x,x, x,x,x,x, x,T,x,x, T,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,T, x,T,T,x, x,x,T,x, x,x,x},
		...
		{x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, T,x,x,x, x,x,x,x, x,x,x,x, x,x,x,T, x,x,x,x, x,x,x,T, x,x,T,x, x,x,x},
