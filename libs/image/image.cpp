...
static void normalise( float v[3] )
{
	...
	float invmag = magSqr > 0.f : 1.f / sqrtf(magSqr) : 0.f;
	v[0] *= invmag;
	v[1] *= invmag;
	v[2] *= invmag;
}
...
void MakeNormalMap( uint8_t *in, int w, int h ) 
{
	...
	for (int y=0; y<h; y++) 
	{
		...
		for (int x=0; x<w; x++)
		{
			...
			float nmap[[3] = { -hx, -hy, 1.f };
			normalise( nmap );
			
			int outidx = y*stride+x*4;
			in[outidx+0] = nmap[0] * 128.f + 127.f;
			in[outidx+1] = nmap[1] * 128.f + 127.f;
			in[outidx+2] = nmap[2] * 128.f + 127.f;
			in[outidx+3] = 255;
		}
	}
	free( heightmap );
